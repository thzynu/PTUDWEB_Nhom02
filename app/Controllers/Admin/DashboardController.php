...
class DashboardController extends BaseController
{
    ...
    public function index()
    {
        try {
            ...
            // Get real data for dashboard sections
            $viewData = [];
            $recentPosts = [];
            $recentComments = [];
            $topCommentedPosts = [];
            $postsViews = ['SUM(view)' => 0];
            $postsWithView = [];
            
            // Try to get view analytics data
            try {
                $viewData = $this->getViewAnalytics() ?: [];
            } catch (\Exception $e) {
                error_log("View analytics error: " . $e->getMessage());
                $viewData = [];
            }
            ...
            try {
                ...
                error_log("Recent comments error: " . $e->getMessage());
                $recentComments = [];
            }
            
            // Try to get top commented posts
            try {
                $topCommentedPosts = $this->getTopCommentedPosts() ?: [];
            } catch (\Exception $e) {
                error_log("Top commented posts error: " . $e->getMessage());
                $topCommentedPosts = [];
            }
            
            // Try to get posts with views (most viewed posts)
            try {
                $postsWithView = $this->postModel->getPostsWithViews(10) ?: [];
            } catch (\Exception $e) {
                error_log("Posts with views error: " . $e->getMessage());
                $postsWithView = [];
            }
            
            // Try to get total views
            try {
                $totalViewsResult = $this->postModel->getTotalViews();
                if ($totalViewsResult && is_array($totalViewsResult)) {
                    $postsViews = $totalViewsResult;
                } else {
                    $postsViews = ['SUM(view)' => 0];
                }
            } catch (\Exception $e) {
                error_log("Total views error: " . $e->getMessage());
                $postsViews = ['SUM(view)' => 0];
            }
